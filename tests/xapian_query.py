...
class XapianSearchQueryTestCase(TestCase):
    ...
    def test_build_query_multiple_words_not(self):
        ...
        self.assertEqual(self.sq.build_query().get_description(), 'Xapian::Query(((<alldocuments> AND_NOT hello) AND_NOT world))')
    ...
    def test_build_query_multiple_words_mixed(self):
        ...
        self.sq.add_filter('content', 'hello', use_or=True)
        self.sq.add_filter('content', 'world', use_not=True)
        self.assertEqual(self.sq.build_query().get_description(), 'Xapian::Query(((why OR hello) AND_NOT world))')
    ...
    #     self.assertEqual(self.sq.build_query().get_description(), 'NOT why AND NOT pub_date:..20090210015900 AND author:..david AND created:20090212121300..* AND NOT title:B..* AND NOT id:1 NOT id:2 NOT id:3')
    # 
    # def test_build_query_wildcard_filter_types(self):
    #     self.sq.add_filter('content', 'why')
    #     self.sq.add_filter('title__startswith', 'haystack')
    #     self.assertEqual(self.sq.build_query().get_description(), 'why AND title:haystack*')
    ...
    #     self.assertEqual(self.sq.build_query().get_description(), '(hello) django_ct:tests.mockmodel')
    # 
    #     self.sq.add_model(AnotherMockModel)
    #     self.assertEqual(self.sq.build_query().get_description(), '(hello) django_ct:tests.anothermockmodel django_ct:tests.mockmodel')
    # 
    # def test_build_query_with_datetime(self):
    #     self.sq.add_filter('pub_date', datetime.datetime(2009, 5, 9, 16, 20))
    #     self.assertEqual(self.sq.build_query().get_description(), u'pub_date:20090509162000')
    # 
    # def test_build_query_with_sequence_and_filter_not_in(self):
    #     self.sq.add_filter('id__exact', [1, 2, 3])
    #     self.assertEqual(self.sq.build_query().get_description(), u'id:[1, 2, 3]')